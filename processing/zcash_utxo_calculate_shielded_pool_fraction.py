...
args = parser.parse_args('--nx_gpickle_file graph/graph_utxo_zcash_spb.gpickle'.split(' '))
...
# %%
i=absorber_recode[('shielded_pool',0)]-Q.shape[1]
...
# %%
entropies = np.load('zcash_utxo/entropies.npy')
unspent = np.load('zcash_utxo/unspent.npy')
...
# %%
import pickle
with open('zcash_utxo/recode', 'rb') as handle:
    recode = pickle.load(handle)

# %%
shielded_pool_id = recode[('shielded_pool',0)]
...
# %%
np.save('zcash_utxo/entropies_modified', entropies_modified)
